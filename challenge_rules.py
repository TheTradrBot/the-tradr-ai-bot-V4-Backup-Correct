...
def format_challenge_summary(result: ChallengeSimulationResult) -> str:
    """Format challenge simulation result for Discord output."""
    lines = [
        "=" * 50,
        "**5ERS HIGH STAKES 10K CHALLENGE SIMULATION**",
        "=" * 50,
        "",
        f"**Period:** {result.period_start.strftime('%b %Y')} - {result.period_end.strftime('%b %Y')}",
        f"**Account Size:** ${FIVERS_10K_RULES.account_size:,.0f}",
        f"**Risk Per Trade:** {FIVERS_10K_RULES.risk_per_trade_pct}%",
        "",
        "-" * 50,
        "**RESULTS SUMMARY**",
        "-" * 50,
        "",
        f"**Full Challenges Passed:** {result.full_challenges_passed}",
        ...
        f"**Total Profit:** ${result.total_profit_usd:+,.2f} ({result.total_profit_pct:+.1f}%)",
        "",
    ]
